...
In order to update the client code generated by openapi to match the EventsAPI you can use the commands below. This process creates a docker container with java which runs the openapi-generator-cli commands. 

To update the client code, the command needs to point to the EventsAPI server. However, if the server is being run locally, replace `http://localhost` with `http://host.docker.internal`. To generate client code based on a different openapi source file replace the argument following -i in the command below with the file path or url. 

```bash
$ cd k12-apps-raise
$ docker run --rm -v "${PWD}:/local" openapitools/openapi-generator-cli generate \
  -i http://host.docker.internal:8888/openapi.json \
  -g typescript-fetch \

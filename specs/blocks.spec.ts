...
test('Math field input using virtual keyboard buttons', async ({ page }) => {
  const htmlContent = `
  <div class="os-raise-ib-pset" data-schema-version="1.0" data-retry-limit="3">
    <div class="os-raise-ib-pset-problem" data-problem-type="input" data-solution='32' data-problem-comparator='math'>
      <div class="os-raise-ib-pset-problem-content">
        <p id="problem">MultipleChoice problem content: \\( x^2 \\)</p>
      </div>
      <div class="os-raise-ib-pset-encourage-response" data-answer="41">
  ...
  await page.locator('math-field').click()
  await page.waitForSelector('.ML__keyboard')
  await page.waitForSelector('.MLK__keycap')
  const keycaps = await page.$$('.MLK__keycap')
  if (keycaps !== null) {
    for (const keycap of keycaps) {
      const textContent = (await keycap.textContent()) ?? ''

...
* Valid values for `data-problem-comparator` include: `integer`, `float`, `text`, and `math`
  ...
  * If the comparator is `math`, the user will have access to an equation editor keyboard. User input will be converted to values that will be compared for mathamatical equvalancy. Example: `x>5` is equivalant to `5<x`
